...
is created and compressed off-chain, then pushed to the "name" field of each tile on chain for storage. Then, for rendering, it is decompressed into its voxel-by-voxel 
"bitmap" form, then rendered into Three.js shapes for inclusion in the Etheria "scene". (Note the lack of physical rules. The tile is simply a canvas now.)
...
It serves as verification that the old mechanics did work (at least somewhat), so nobody has to take my word for it. The data is there and always will be (unless the current 
tile owner moves those old bricks around).

HowTo

This was all built with express.js back in the day when I was a much worse programmer so I'm not exactly sure how it works.

I think, though, you just get the current map state (with bricks) into a file called /map. This was generated at server startup time back then, but I think you can just 
pull the state, put it at /map, and it should work.
